...
class SuffixArray(object):
    ...
    def __init__(self, string):
        self._string = string
        indexes = sorted([(string[i:], i) for i in xrange(len(string))])
        self._suffix_array = [si[1] for si in indexes]
    ...
    #
    def search(self, SA, T, W, verbose=True):
        ...
        l = LCP(calcLCP(W, self._string[SA[0]:]), 'l')
        r = LCP(calcLCP(W, self._string[SA[N - 1]:]), 'r')
        LCP1 = naiveLCP1(self._string, SA)
        ...
        if l.length == p or (len(self._string[SA[0]:]) > p and W[l.length] <= self._string[SA[0] + l.length]):
            LW = 0
        elif r.length < p and W[r.length] > self._string[SA[N - 1] + r.length]:
            LW = N
        else:
            L, R = 0, N
            while R - L > 1:
                ...
                if l.length >= r.length:
                    if LCP_LM[M] >= l.length:
                        m = l.length + calcLCP(W[l.length:], self._string[SA[M] + l.length:])
                    else:
                        m = LCP_LM[M - 1]
                else:
                    if LCP_MR[M - 1] >= r.length:
                        m = r.length + calcLCP(W[r.length:], self._string[SA[M] + r.length:])
                    else:
                        m = LCP_MR[M - 1]
                if m == p or (len(self._string[SA[M]:]) > m and W[m] <= self._string[SA[M] + m]):
                    R = M
                    r.length = m
                else:
                    L = M
...
def suffixArray(T):
    sufInds = sorted([(T[i:], i) for i in xrange(len(T))])
...
def naiveLCP1(t, sa):
    return [calcLCP(t[sa[i]:], t[sa[i + 1]:]) for i in xrange(len(sa) - 1)]
